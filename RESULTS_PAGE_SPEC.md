...
## 2.5 主敘事骨幹怎麼形成（triangulation）

**所有敘事(標題／總綱／主敘事／未來事件)都是同一次 `askOracle` 呼叫、一份 JSON 產生的**，不是分開生成。形成流程：

1. **匯流**：所有來源先攤平成「編號證據池」(`[W#]` web／`[M#]` 市場賠率／`[N#]` 新聞／`[H#]` HN)＋ 5 個 scout 的解讀＋Trends／Wiki，**同一個模型同時看到全部** → 這就是 triangulation 的基礎(不是各來源各自結論，而是一個大腦交叉比對)。
2. **角色**：領域專家(`expert`)＋未來學家雙重身分。
3. **reasoning-first 排序(最關鍵)**：prompt 強制「**先把 drivers／depth／crossImpacts 想清楚，再『據此』推導 horizons**」→ 未來錐**長在分析之上**，不是各自獨立。
4. **跨域交叉連結**：明令「把不同 scout 的訊號**串起來、找綜效/張力**，不要分開條列」(政策×市場、研究×社群…)。
5. **主敘事(`narrative`)＝骨幹**：3–5 句，把「最新＋最相關＋賠率最硬」的證據織成一條連貫主線(現在發生什麼、誰是關鍵、哪些是最重要驅動)；緊扣真實情報(尤其 web 發現與賠率)，並**定錨**後面的 drivers/horizons(它們都要與這條一致)。

**欄位 → 頁面**：`topic`＝標題 · `summary`＝一句富畫面感的總綱預言 · `narrative`＝「現況主敘事」塊 · `drivers`＝關鍵驅動卡 · `horizons.events`＝未來錐光點＋三段時間卡。
